...
function AddScheduleModal({
  ...
  const handleColorSelect = (cat) => {
    const updateData = { color: cat.color };
    
    // 숫자가 있는 색상(1-6번)의 경우 시간도 자동 설정
    if (cat.number && cat.startTime && cat.endTime) {
      updateData.startTime = cat.startTime;
      updateData.endTime = cat.endTime;
    } else {
          ...
          <div className="flex gap-2 mt-1 flex-wrap">
            {CATEGORY_COLORS.map((cat) => (
              <button
                type="button"
                key={cat.color}
                className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-xs font-bold ${formData.color === cat.color ? "ring-2 ring-blue-400" : ""}`}
                style={{ 
