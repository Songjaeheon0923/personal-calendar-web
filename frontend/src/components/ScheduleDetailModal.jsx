...
import { formatTimeInput, validateAndLimitEndTime } from "../utils/timeUtils";
...
function ScheduleDetailModal({
  ...
  const handleEndTimeChange = (e) => {
    const formatted = validateAndLimitEndTime(e.target.value);
    onFormDataChange({ editEndTime: formatted });
  };

  // 색상 선택 핸들러 (시간 자동 설정 포함)
  const handleColorSelect = (cat) => {
    const updateData = { editColor: cat.color };
    
    // 숫자가 있는 색상(1-6번)의 경우 시간도 자동 설정
    if (cat.number && cat.startTime && cat.endTime) {
      updateData.editStartTime = cat.startTime;
      updateData.editEndTime = cat.endTime;
    } else {
      // 7-8번 색상의 경우 시간을 공백으로 초기화
      updateData.editStartTime = '';
      updateData.editEndTime = '';
    }
            ...
            <div className="flex gap-2">
              {CATEGORY_COLORS.map(cat => (
                <button
                  type="button"
                  key={cat.color}
                  className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-xs font-bold ${formData.editColor === cat.color ? "ring-2 ring-blue-400" : ""}`}
                  style={{ 
                    background: cat.color,
                    color: cat.number ? (cat.color === '#ffe066' ? '#333' : '#fff') : 'transparent'
                  }}
                  onClick={() => handleColorSelect(cat)}
                  aria-label={cat.name}
                >
                  {cat.number || ''}
                </button>
